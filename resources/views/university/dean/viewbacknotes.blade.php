@extends('templates.dean.main',['title' => 'View Backnotes'])
	...
	<tbody>
		...
		<tr {{ $backnote->user_type == "dean" ? "class=\"active\"" : "" }}>
			<td><strong>{{ $backnote->user->fullname() }}</strong>
				...
				<br/>
				<br/>
				<a class="btn btn-success btn-xs" href="{{ route('deanviewinspection',$backnote->id_inspection) }}" target="_blank">View Inspection no. {{ $backnote->id_inspection }}</a>
				@endif
				<br/>
				<br/>
...
<div class=" col-md-12" style="text-align: center;"> 
	<hr class="redhr">
	<a class="btn btn-primary" href="{{ route('deanbacknotespdf',$collegeid->id) }}">Download PDF</a>
</div>
@else
<table width="100%">
	<tr>
	...
	<tr style="line-height:30px;">
		...
		<td>
			<label>
				@if($college->form)
				<a class="btn btn-primary btn-xs" href="{{ route('deanviewapplication',$college->form->id) }}">View Application
				</a>
				@else
		...
		<td>
			<label>
				<a class="btn btn-primary btn-xs" href="{{ route('deanbacknotes',$college->id) }}">View Backnotes</a>
			</label>
		</td>
	</tr>
	@endif
